...
try:
    ...
    client_socket = None  # Set to None if connection fails
...
BASE_SPEED = 20 # Set your desired base speed here
current_speed = BASE_SPEED
SENSITIVITY_THRESHOLD = 4
TURN_FACTOR = 0.5
Points_updater = Thread(target=Update_points)

def send_data(frame, point):
        # Serialize the frame
        ...
        try:
            ...
            # Send 3D points
            if point:
                points_data = pickle.dumps(point)
...
try:
    ...
    control_motors(BASE_SPEED, BASE_SPEED)
    while True:
        ...
        if client_socket:
            try:
                latest_point = points_3d[-1] if points_3d else None
                send_data(output_frame, latest_point) #passing frame and points
            except Exception as e:
                print(f"Error sending data: {e}")
            ...
            if client_socket:
                ...
                if command_data:
                    ...
                    if command_data.startswith("speed="):
                        BASE_SPEED = int(command_data.split("=")[1])
                        print(f"Received speed: {BASE_SPEED}")
                        control_motors(BASE_SPEED,BASE_SPEED)
                    elif command_data == "stop":
                        control_motors(0,0)
                        BASE_SPEED = 0
                    elif command_data == "start":
                         BASE_SPEED = 20
                         control_motors(BASE_SPEED,BASE_SPEED)
